...
impl RustBlogPage {
    pub fn new() -> Self {
        Self {
            target_url: String::from(RUST_BLOG_PAGE_URL),
        }
    }
    ...
    fn html_body_to_article(body: String) -> Result<Vec<Article>, Box<dyn std::error::Error>> {
        ...
        let articles: Option<Vec<Article>> = tr_elements.map(|tr_element| {
            let td_elements: Vec<_> = tr_element.child_elements().collect();
            
            let published_at = Self::get_published_at(&td_elements)?;
            let title = Self::get_title(&td_elements)?;
            let url_path = Self::get_url_path(&td_elements)?;
    ...
    pub async fn build_article(&self) -> Result<Vec<Article>, Box<dyn std::error::Error>> {
        let body = reqwest::get(self.target())
            .await?
            .text()
            .await?;

        let articles = Self::html_body_to_article(body)?;

        Ok(articles)
    }
}

impl Page for RustBlogPage {
